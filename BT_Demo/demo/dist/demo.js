...
class Product {
    ...
    sell(quantity) {
        ...
        this.stock -= quantity;
        console.log(`Đã bán ${quantity} sản phẩm. Số lượng tồn kho còn lại: ${this.stock}`);
    }
    restock(quantity) {
        if (quantity <= 0) {
        ...
        this.stock += quantity;
        console.log(`Đã nhập thêm ${quantity} sản phẩm. Tồn kho hiện tại: ${this.stock}`);
    }
}
Product.nextId = 1;
...
class Store {
    ...
    createOrder(customerId, productQuantities) {
        ...
        for (const pq of productQuantities) {
            ...
            if (product.stock < pq.quantity) {
                console.log(`Sản phẩm "${product.name}" không đủ hàng. Chỉ còn ${product.stock}.`);
                continue;
            }
            ...
            console.log(`Đơn hàng ID ${newOrder.orderId} đã được tạo cho khách hàng "${customer.name}"`);
            return newOrder;
        }
    }
    cancelOrder(orderId) {
    ...
    listAvailableProducts() {
        ...
        availableProducts.forEach((product) => {
            console.log(`- ${product.name} | Giá: ${product.price} | Tồn kho: ${product.stock}`);
        });
    }
    listCustomerOrders(customerId) {
        const customerOrders = this.orders.filter((order) => order.customer.id === customerId);
        if (customerOrders.length === 0) {
            console.log(`Không tìm thấy đơn hàng nào của khách hàng ID: ${customerId}`);
            return;
        }
    ...
    updateProductStock(productId, newStock) {
        ...
        if (index !== -1) {
            ...
            console.log(`Đã cập nhật tồn kho cho sản phẩm ID ${productId} thành ${newStock}`);
        }
        else {
